...
{"line_items": [{"blueprint_id": {{ printify_params.blueprint_id}}, "price": {{ printify_params.price }}, "print_provider_id": {{ printify_params.print_provider_id}}, "variant_id": {{ printify_params.variant_id}}, "print_areas": {"front": "{{ front_img_url }}"}, "quantity": 1}], "external_id": "{{ order_id }}", "shipping_method": 1, "address_to": {"phone": "{{ shipping.phone }}", "first_name": "{{ shipping.first_name }}", "last_name": "{{ shipping.last_name }}", "address2": "{{ shipping.addr2 }}", "zip": "{{ shipping.zip }}", "address1": "{{ shipping.addr1 }}", "country": "{{ shipping.country }}", "region": "{{ shipping.state }}", "city": "{{ shipping.city }}", "email": "{{ shipping.email }}"}, "send_shipping_notification": false}
""")
...
class Printify(object):
    ...
    def create_draft_order(self, front_img_url, shipping_address):
        order_id = self._generate_order_id()
        body = ORDER_TEMPLATE.render(order_id=order_id, front_img_url=front_img_url,
                              printify_params=self._printify_params,
                              shipping=shipping_address)
        print body
        r = requests.post('https://api.printify.com/v1/shops/%s/orders.json' % self._printify_params['draft_store_id'],
                          data=body, headers = self._headers)
        if r.status_code == 200:
            print r.json()
            printify_order = r.json()['id']
            r = requests.get('https://api.printify.com/v1/shops/%s/orders/%s.json' % (self._printify_params['draft_store_id'], printify_order),
            ...
            print 'ERROR CREATING DRAFT ORDER'
            print r.status_code
            print r.text
            return None
